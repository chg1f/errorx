...
func TestWrapAndIs(t *testing.T) {
	...
	err := errorx.WithCode("missing").WithMessage("load config").Wrap(base)
...
func TestPackageLevelConstructors(t *testing.T) {
	...
	assert.EqualError(t, errorx.Errorf("hello %s", "world"), "hello world")

	joined := errorx.Joins(errors.New("left"), errors.New("right"))
	require.Error(t, joined)
	assert.EqualError(t, joined, "left;right")

	joined = errorx.WithCode("invalid").Joins(errors.New("left"), nil, errors.New("right"))
	require.Error(t, joined)
	assert.EqualError(t, joined, "#invalid left;right")

	builder := errorx.WithMessage("message").WithValues(map[string]any{"k": "v"})
	err := builder.WithCode(errorx.Unspecified).New("override")
	assert.EqualError(t, err, "override")
}

// TestI18n verifies message resolution is locale-key based and locale-aware.
func TestI18n(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(path, []byte(`[
  {
	...
	assert.Equal(t, "", ex.Localize(language.MustParse("en")))
